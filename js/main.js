...
function generateDisplayTableFun() {
    ...
    if (counter >= 3) {
        if (searchBar.value != '') {
            (
                async function () {
                    const req = await fetch(`https://api.weatherapi.com/v1/search.json?key=f2884e07136c4a3b954230649241001&id=2657545&q=${searchBar.value}`);
                    if (req.status == 200) {
...
function getDegree(whichCity) {
    ...
    req.open('get', `https://api.weatherapi.com/v1/forecast.json?key=f2884e07136c4a3b954230649241001&q=${whichCity}&days=3`);
    req.send();
    req.addEventListener('loadend', function () {
        if (req.status >= 200 && req.status <= 300) {
            ...
            if (Number(finalResult.current.temp_c) >= 24) {
                mainApp.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.250) 50%, rgba(0, 0, 0, 0.250) 50%), url(./img/banner.png)`;
